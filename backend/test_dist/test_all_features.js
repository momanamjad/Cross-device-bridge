    ...
    try {
        ...
        const regAndroidRes = await fetch(baseUrl + "/api/devices/register", {
            ...
            headers: {
                ...
                "X-Register-Secret": environment_1.env.registerSecret,
            },
            body: JSON.stringify({
        ...
        if (regIphoneJson.data) {
            const decrypted = (0, crypto_1.decryptPayload)(regIphoneJson.data, environment_1.env.registerSecret);
        ...
        androidSocket = (0, socket_io_client_1.io)(baseUrl, {
            auth: { token: androidToken },
            transports: ["websocket"],
        });
        iphoneSocket = (0, socket_io_client_1.io)(baseUrl, {
        ...
        const smsPromise = new Promise((resolve, reject) => {
            ...
            iphoneSocket.on("message:new", (envelope) => {
                clearTimeout(timeout);
                try {
                    const payload = envelope.data ? (0, crypto_1.decryptPayload)(envelope.data, environment_1.env.registerSecret) : envelope;
                    resolve(payload);
                }
                catch (e) {
                    reject(e);
        ...
        if (receivedSms.sender === testSmsSender && (receivedSms.message === testSmsBody || receivedSms.content === testSmsBody)) {
        ...
        if (fetchSmsJson.data) {
            const decrypted = (0, crypto_1.decryptPayload)(fetchSmsJson.data, environment_1.env.registerSecret);
        ...
        const incomingCallPromise = new Promise((resolve, reject) => {
            ...
            iphoneSocket.on("call:incoming", (envelope) => {
                clearTimeout(timeout);
                try {
                    const payload = envelope.data ? (0, crypto_1.decryptPayload)(envelope.data, environment_1.env.registerSecret) : envelope;
                    resolve(payload);
        ...
        const outgoingCallPromise = new Promise((resolve, reject) => {
            const timeout = setTimeout(() => reject(new Error("Timeout waiting for call:outgoing on Android")), 5000);
            androidSocket.on("call:outgoing", (envelope) => {
                clearTimeout(timeout);
                try {
                    const payload = envelope.data ? (0, crypto_1.decryptPayload)(envelope.data, environment_1.env.registerSecret) : envelope;
                    resolve(payload);
        ...
        const encryptedOutgoing = (0, crypto_1.encryptPayload)({
            call_id: testCallId,
        ...
        const encryptedHangup = (0, crypto_1.encryptPayload)({ call_id: testCallId }, environment_1.env.registerSecret);
        ...
        let historyList = [];
        if (historyJson.data) {
            const decrypted = (0, crypto_1.decryptPayload)(historyJson.data, environment_1.env.registerSecret);
